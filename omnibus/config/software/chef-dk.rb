...
build do
  ...
  appbundle "chef", lockdir: project_dir, without: %w{integration docgen maintenance travis}, env: env
  appbundle "foodcritic", lockdir: project_dir, without: %w{development}, env: env
  appbundle "inspec", lockdir: project_dir, without: %w{maintenance tools deploy}, env: env

  %w{chef-dk chef-vault ohai test-kitchen opscode-pushy-client cookstyle dco berkshelf}.each do |gem|
